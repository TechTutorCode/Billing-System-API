...
-- ============================================
-- 7. ADD password_hash COLUMN (if table already exists)
-- ============================================
-- If the customers table already exists, add the password_hash column
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        -- Set default password (account_number) for existing customers
        -- Note: Password hashing must be done in the application using bcrypt
        -- This migration sets a placeholder that needs to be updated
        -- You should run a script to hash all passwords after this migration
        UPDATE customers
        SET password_hash = account_number  -- Placeholder: must be hashed with bcrypt
        WHERE password_hash IS NULL;
        
        -- Make password_hash NOT NULL after populating
        ALTER TABLE customers ALTER COLUMN password_hash SET NOT NULL;
        
        -- IMPORTANT: After running this migration, update all password_hash values
        -- using bcrypt.hash_password(account_number) in your application
        -- Example Python script:
        -- from app.auth.utils import hash_password
        -- for customer in db.query(Customer).all():
        --     customer.password_hash = hash_password(customer.account_number)
        -- db.commit()
    END IF;
END $$;
